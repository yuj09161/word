...
import selectUI,settingUI,testUI,selectTestUI
...
class selectWin(QMainWindow,selectUI.SelectUI):
    def __init__(self):
        def setting():
            if setting_closed:
                self.__setting.show()
...
class settingWin(QDialog, settingUI.SettingUI):
...
class testWin(QMainWindow, testUI.TestUI):
...
class selectTestWin(QMainWindow, selectTestUI.SelectTestUI):
